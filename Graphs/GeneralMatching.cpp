...
struct MaxMatching {
	int N; vector<vi> adj;
	vector<int> mate, first; vector<bool> white; vector<pair<int,int>> label;
	//initialize
	void init(int _N) { N = _N; adj = vector<vi>(N+1);
		mate = first = vi(N+1); label = vector<pair<int,int>>(N+1); white = vector<bool>(N+1); }
    //add edge
		...
		while (!q.empty()) {
			...
			for(auto& b : adj[a]) { assert(b);
                if(white[b]) {
                    ...
                    mate[b] = a; match(a,b); white = vector<bool>(N+1); // reset
					return true;
